class OrderFoodItemsController < ApplicationController
  ...
  def create
    @food_item = OrderFoodItem.new(food_item_params)
  ...
  def destroy
    @food_item.destroy
    respond_to do |format|
      format.html { redirect_to order_food_items_url, notice: 'Food item was successfully destroyed.' }
  ...
  private
    ...
    def set_food_item
      @food_item = OrderFoodItem.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def food_item_params
      params.require(:order_food_item).permit(:name, :price, :description_id, :image_url,
